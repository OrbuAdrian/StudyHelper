...
function loadState() {
    ...
    try {
      ...
      merged.quizDraft = normalizeQuizProblems(stored.quizDraft || [], merged.exercises);
      merged.quizzes = merged.quizzes.map(quiz => ({
        ...quiz,
        problems: normalizeQuizProblems(quiz.problems || quiz.exercises || [], merged.exercises)
      }));
...
function isSemanticType(type) {
  return ['semantic', 'semantic-explanation', 'definition', 'comparison', 'reasoning', 'phrase-completion', 'valid-statement'].includes(type);
}
...
function generateTemplateExercise() {
    ...
    try {
      ...
      const exercise = {
        ...
        source: 'template',
        sourceTitle: result.metadata.TITLE || els.templateName.value.trim() || 'Exercise template',
        templateText: text,
        createdAt: new Date().toISOString()
      };
      state.currentExercise = exercise; renderExerciseCard(exercise, els.templateExerciseHost);
      toast('Exercise generated', `A reproducible template instance was created with seed ${result.seed}.`, 'success');
    } catch (error) { renderExerciseHostError(els.templateExerciseHost, error); toast('Template error', error.message, 'error'); }
  }
...
function saveCurrentTemplate() {
    const name = els.templateName.value.trim() || 'Untitled template'; const text = els.templateText.value.trim();
    ...
    if (!report || !report.valid) return toast('Template not saved', 'Resolve the validator errors before saving this template.', 'error');
    const existingIndex = state.templates.findIndex(item => item.name.toLowerCase() === name.toLowerCase());
    const item = { id: existingIndex >= 0 ? state.templates[existingIndex].id : uid(), name, text, updatedAt: new Date().toISOString() };
    if (existingIndex >= 0) state.templates.splice(existingIndex, 1, item); else state.templates.unshift(item);
    saveState(); renderRecentWork(); toast('Template saved', `${name} is available in your library.`, 'success');
  }
...
function addQuizProblem() {
    if (!state.exercises.length) {
      return toast('Save an exercise first', 'The candidate list is built from exercises in your local library.', 'error');
    }
...
function renderQuizBuilder() {
    ...
    if (!problems.length) {
      ...
      els.quizBuilderList.innerHTML = '<span>✓</span><strong>Your quiz has no problems</strong><p>Save exercises to the library, then add a problem and select its candidate pool.</p>';
      return;
    }
    ...
    els.quizBuilderList.innerHTML = problems.map((problem, index) => {
      ...
      const candidateCount = getProblemCandidates(problem, state.exercises).length;
    ...
    els.quizBuilderList.querySelectorAll('[data-index]').forEach(row => {
      ...
      row.querySelector('[data-role="candidate-select"]').addEventListener('change', event => {
        ...
        state.quizDraft[index] = setProblemCandidates(state.quizDraft[index], candidateIds, state.exercises);
        saveState();
        renderQuizBuilder();
      });
...
function createResolvedQuiz(blueprint = getQuizDraftObject()) {
    const exercises = resolveQuizProblems(blueprint.problems || [], state.exercises);
    return {
      ...clone(blueprint),
...
function openLibraryItem(item, type) {
    if (type === 'summaries') { state.currentSummary = clone(item); renderSummaryOutput(); navigateTo('summary'); }
      ...
      state.quizDraft = normalizeQuizProblems(item.problems || item.exercises || [], state.exercises);
...
function applyStoredStateToUI() {
    ...
    updateSourceCharCount();
    const key = state.settings.apiKey || sessionStorage.getItem(SESSION_KEY) || '';
    ...
    const validationInput = document.querySelector(`input[name="validationMode"][value="${CSS.escape(state.settings.validationMode || 'combined')}"]`);
...
function updateStorageIndicators() {
    const bytes = new Blob([localStorage.getItem(STORAGE_KEY) || '']).size; const label = formatBytes(bytes);
...
function getApiKey() {
    return (state.settings.apiKey || sessionStorage.getItem(SESSION_KEY) || '').trim();
  }
...
function exportWorkspace() {
    ...
    downloadJson(`study-forge-workspace-${new Date().toISOString().slice(0, 10)}.json`, {
      app: 'Study Forge', version: 3, exportedAt: new Date().toISOString(), data: exported
    });
...
async function importWorkspace(file) {
    ...
    try {
      ...
      state.quizDraft = normalizeQuizProblems(incoming.quizDraft || [], state.exercises);
      state.quizzes = state.quizzes.map(quiz => ({
        ...quiz,
        problems: normalizeQuizProblems(quiz.problems || quiz.exercises || [], state.exercises)
      }));
