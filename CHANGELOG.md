...
- Added a dedicated Notes view with optional titles, bilingual content, editing, deletion, and local-library persistence.
- Added notes to workspace import/export, recent work, statistics, and the Library.
- Added `memo` as a third flashcard type.
- Memo cards are generated by Gemini from the meaning of saved notes and use complete questions with 3–5 selectable choices.
- Long notes are processed through paragraph-aware batches so Gemini may create one or several cards without requiring one oversized request.
- Memo-card answers are validated locally and cycle through option positions on every review.
- Memo cards retain source note IDs and survive later note editing or deletion.
- Workspace export schema advanced to version 8.
